...
const fetchFortune = async (userAnswer) => {
    try {
        ...
        return "Sorry, I couldn't generate your fortune at this time.";
    }
};
    ...
    return new Promise((resolve) => {
        const handleKeydown = (e) => {
            if (e.key === "Enter") {
                ...
                if (input) {
                    const userMsgElement = createMessageElement(`<div class="message-text">${input}</div>`, "user-message");
...
const conversationLoop = async () => {
    ...
    const fortune = await fetchFortune(answer);

    // Remove the thinking message before showing the response
    chatBody.removeChild(thinkingMsgElement);

    addBotMessage(fortune);
    addBotMessage("Would you like another question? Type 'continue' to keep going or 'quit' to end the chat.");

    const followUp = await waitForUserInput();
...
const handleOutgoingMessage = async (userMessage) => {
    ...
    const fortune = await fetchFortune(userMessage);
    addBotMessage(fortune);
};
...
messageInput.addEventListener("keydown", async (e) => {
    ...
    if (e.key === "Enter" && userMessage) {
        await handleOutgoingMessage(userMessage);
    }
});

// On page load, greet the user and start the chatbot
...
//                 const input = messageInput.value.trim();
//                     if(input){
//                         createMessageElement(`<div class="message-text">${input}</div>`, "user-message");
//                         messageInput.value = "";
//                         document.removeEventListener("keydown", handleKeydown);
//                         resolve(input);
//                     }
